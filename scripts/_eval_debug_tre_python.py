...
def main():
    parser = argparse.ArgumentParser(description="Post-hoc Python TRE evaluation for debug iteration STLs.")
    parser.add_argument("debug_dir")
    parser.add_argument("--data-root", default=case_config.DATA_ROOT)
    parser.add_argument("--patient-id", default=case_config.PATIENT_ID)
    parser.add_argument("--frame-id", default=case_config.FRAME_ID)
    parser.add_argument("--sample-count", type=int, default=50)
    parser.add_argument("--icp-iterations", type=int, default=10)
    parser.add_argument("--oncologic-margin", type=float, default=10.0)
    ...
    rows = eval_debug_dir(
        ...
        args.sample_count,
        args.icp_iterations,
        args.oncologic_margin,
    )
    out = args.out or os.path.join(args.debug_dir, "python_tre.csv")
    write_csv(out, rows)
    best = min(rows, key=lambda row: row["TRE_mm"])
    for row in rows:
        print(f"iter_{row['iter']:02d}: TRE={row['TRE_mm']:.6f} mm, IC={row['IC']}, file={row['file']}")
    print(f"[OK] best iter_{best['iter']:02d}: TRE={best['TRE_mm']:.6f} mm, IC={best['IC']}")
    print(f"[OK] CSV: {out}")
