...
当前仓库包含 scripts 01 和 08 所需的 Patient1 源文件：
...
当前刚性流程未直接读取的其他已发布 Patient1 源文件仍保留，用于审计和未来
验证。不得将这些文件与生成的 `data_case/` 数据混淆。

帧 `02`、`03`、`04`、`06`、`07`、`08`、`09` 和 `10` 具备完整源输入。
当前提供的源数据子集中没有帧 `05`，script 08 会将其报告为输入不完整。
